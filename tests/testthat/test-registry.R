...
test_that("registry helpers reject unknown or malformed metric names", {
  ...
  expect_error(ts_metric(x, "not_a_metric"), "Unknown metric")
  expect_error(ts_metric(x, c("total_time", "n_periods")), "single metric name")
  expect_error(ts_metrics(x, c("total_time", NA)), "character vector")
})
